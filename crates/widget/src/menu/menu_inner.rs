...
impl MenuBounds {
    #[allow(clippy::too_many_arguments)]
    fn new<'a, Message, Theme, Renderer>(
...
impl MenuState {
        ...
        Renderer: renderer::Renderer,
    {
        ...
        let child_nodes = self.menu_bounds.child_positions[start_index..=end_index]
            ...
            .zip(menu_tree[start_index..=end_index].iter_mut())
            .map(|((cp, size), mt)| {
                let mut position = *cp;
                let mut size = *size;
...
pub(super) struct Menu<'a, 'b, Message, Theme = crate::Theme, Renderer = crate::Renderer>
where
    Message: Clone,
    Renderer: renderer::Renderer,
    Theme: StyleSheet,
{
    pub(super) tree: MenuBarState,
    pub(super) menu_roots: &'b mut [MenuTree<'a, Message, Theme, Renderer>],
    pub(super) bounds_expand: u16,
    pub(super) menu_overlays_parent: bool,
            ...
            let children: Vec<Node> = data
                ...
                .filter_map(|(i, ms)| {
                    ...
                    let roots = active_root.iter().skip(1).fold(
                        &mut menu_roots[active_root[0]].children,
                        |mt, next_active_root| &mut mt[*next_active_root].children,
                    );
                ...
                state.menu_states.iter().zip(layout.children()).enumerate()
            {
                ...
                renderer.with_layer(render_bounds, |r| {
                    ...
                    if start_index < roots.len() {
                        for (mt, clo) in roots[start_index..=end_index]
                            .iter()
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn init_root_menu<'a, 'b, Message, Theme, Renderer>(
...
#[allow(clippy::too_many_arguments)]
fn process_menu_events<'a, 'b, Message, Theme, Renderer>(
    menu: &mut Menu<'a, 'b, Message, Theme, Renderer>,
    event: &event::Event,
    view_cursor: Cursor,
    renderer: &Renderer,
...
#[allow(clippy::too_many_arguments)]
fn process_overlay_events<'a, 'b, Message, Theme, Renderer>(
    menu: &mut Menu<'a, 'b, Message, Theme, Renderer>,
    renderer: &Renderer,
    layout: Layout<'_>,
    viewport_size: Size,
    ...
    Theme: StyleSheet,
{
    ...
    menu.tree.inner.with_data_mut(|state| {
        ...
        let last_menu_state = &mut state.menu_states[last_menu_state_index];
        ...
        let roots = active_root.iter().skip(1).fold(
            &mut menu.menu_roots[active_root[0]].children,
            |mt, next_active_root| &mut mt[*next_active_root].children,
        );
        ...
        let item = &mut active_menu[new_index];
...
fn process_scroll_events<'a, 'b, Message, Theme, Renderer>(
    menu: &mut Menu<'a, 'b, Message, Theme, Renderer>,
...
fn get_children_layout<'a, Message, Theme, Renderer>(
    menu_tree: &mut MenuTree<'a, Message, Theme, Renderer>,
    renderer: &Renderer,
    item_width: ItemWidth,
    item_height: ItemHeight,
